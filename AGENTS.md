...
- Use `bun` only. The lockfile is `bun.lock`.
- Dev server: `bun run dev`
- Production build: `bun run build`
- Production server: `bun run start`
- Lint: `bun run lint`
- Format whole repo: `bun run format`
- Convex: `bun run convex:dev`, `bun run convex:push`, `bun run convex:deploy`

## Verification

- There is no test suite and no `typecheck` script.
- The meaningful verification path is `bun run build && bun run lint`.
- Pre-commit only runs `bunx lint-staged`, which formats staged files with Prettier.
...
- run `bunx react-doctor@latest` and secure everything is fine
